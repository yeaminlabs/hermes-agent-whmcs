...
.hchat-shell { display: flex; flex-direction: column; flex: 1; overflow: hidden; background: #fff; border: 1px solid #e5e7eb; border-radius: 16px; box-shadow: 0 4px 24px rgba(0,0,0,.06); }
.hchat-header { padding: 16px 20px; border-bottom: 1px solid #f3f4f6; display: flex; align-items: center; justify-content: space-between; flex-shrink: 0; background: #fff; border-radius: 16px 16px 0 0; }
.hchat-header-left { display: flex; align-items: center; gap: 12px; }
.hchat-avatar { width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg,#CC0000,#ff4444); display: flex; align-items: center; justify-content: center; color: #fff; font-size: 18px; flex-shrink: 0; }
.hchat-agent-name { font-size: 16px; font-weight: 700; color: #111827; }
.hchat-agent-sub { font-size: 12px; color: #6b7280; margin-top: 1px; }
.hchat-online-dot { width: 8px; height: 8px; border-radius: 50%; background: #10b981; display: inline-block; margin-right: 5px; animation: hchat-pulse 2s infinite; }
@keyframes hchat-pulse { 0%,100%{opacity:1} 50%{opacity:.4} }
.hchat-actions { display: flex; gap: 8px; align-items: center; }
.hchat-btn { padding: 6px 14px; border-radius: 8px; font-size: 12px; font-weight: 700; cursor: pointer; border: 1px solid #e5e7eb; background: #f9fafb; color: #374151; display: flex; align-items: center; gap: 5px; transition: all .15s; font-family: 'Outfit', sans-serif; }
.hchat-btn:hover { background: #f3f4f6; }
.hchat-messages { flex: 1; overflow-y: auto; padding: 24px 20px; display: flex; flex-direction: column; gap: 16px; scroll-behavior: smooth; scrollbar-width: thin; scrollbar-color: #e5e7eb transparent; }
...
.hchat-bubble-avatar { width: 30px; height: 30px; border-radius: 50%; flex-shrink: 0; display: flex; align-items: center; justify-content: center; font-size: 14px; }
.hchat-row.user .hchat-bubble-avatar { background: #dbeafe; color: #1d4ed8; }
.hchat-row.agent .hchat-bubble-avatar { background: linear-gradient(135deg,#CC0000,#ff4444); color: #fff; }
.hchat-bubble { padding: 12px 16px; border-radius: 16px; font-size: 14px; line-height: 1.6; max-width: 100%; word-break: break-word; }
.hchat-row.user .hchat-bubble { background: #1d4ed8; color: #fff; border-bottom-right-radius: 4px; }
.hchat-row.agent .hchat-bubble { background: #f9fafb; color: #111827; border: 1px solid #f3f4f6; border-bottom-left-radius: 4px; }
...
.hchat-bubble code { background: #e5e7eb; padding: 1px 5px; border-radius: 4px; font-size: 12px; font-family: monospace; }
.hchat-row.user .hchat-bubble code { background: rgba(255,255,255,.25); }
.hchat-bubble pre { background: #1e293b; color: #e2e8f0; padding: 12px 14px; border-radius: 8px; overflow-x: auto; margin: 8px 0; font-size: 12px; font-family: monospace; line-height: 1.5; }
...
.hchat-bubble blockquote { border-left: 3px solid #d1d5db; padding-left: 10px; color: #6b7280; margin: 6px 0; font-style: italic; }
...
.hchat-typing span { width: 7px; height: 7px; border-radius: 50%; background: #9ca3af; animation: hchat-bounce .8s infinite; }
...
.hchat-empty { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; color: #9ca3af; }
...
.hchat-pill { padding: 7px 14px; border-radius: 100px; font-size: 12px; font-weight: 600; border: 1px solid #e5e7eb; background: #f9fafb; color: #374151; cursor: pointer; transition: all .15s; }
.hchat-pill:hover { background: #CC0000; color: #fff; border-color: #CC0000; }
.hchat-input-area { flex-shrink: 0; padding: 14px 20px; border-top: 1px solid #f3f4f6; background: #fff; border-radius: 0 0 16px 16px; }
.hchat-input-box { display: flex; gap: 10px; align-items: flex-end; background: #f9fafb; border: 1.5px solid #e5e7eb; border-radius: 12px; padding: 10px 14px; transition: border-color .15s; }
.hchat-input-box:focus-within { border-color: #CC0000; background: #fff; box-shadow: 0 0 0 3px rgba(204,0,0,.08); }
.hchat-textarea { flex: 1; border: none; background: transparent; resize: none; outline: none; font-family: 'Outfit', sans-serif; font-size: 14px; color: #111827; line-height: 1.5; max-height: 160px; min-height: 22px; overflow-y: auto; }
.hchat-textarea::placeholder { color: #9ca3af; }
.hchat-send { width: 36px; height: 36px; border-radius: 8px; border: none; cursor: pointer; background: #CC0000; color: #fff; display: flex; align-items: center; justify-content: center; font-size: 14px; flex-shrink: 0; transition: background .15s; align-self: flex-end; }
...
.hchat-error { margin: 0 20px 12px; padding: 10px 14px; border-radius: 8px; background: #fef2f2; border: 1px solid #fecaca; color: #991b1b; font-size: 13px; font-weight: 600; display: flex; align-items: center; gap: 8px; }
.hchat-suspended { background: #fef2f2; padding: 16px; border-radius: 12px; border: 1px solid #fecaca; color: #991b1b; font-weight: 600; font-size: 14px; }
</style>
{/literal}
...
<div class="hchat-shell">
    ...
    <div class="hchat-header">
        <div class="hchat-header-left">
            ...
            <div>
                <div class="hchat-agent-name"><span class="hchat-online-dot"></span>{$agent_name}</div>
                <div class="hchat-agent-sub">Hermes Agent &middot; hermes-{$serviceid}</div>
            </div>
        </div>
        <div class="hchat-actions">
            <button class="hchat-btn" onclick="hchatNewConversation()"><i class="fas fa-plus"></i> New Chat</button>
            <button class="hchat-btn" onclick="hchatExport()"><i class="fas fa-copy"></i></button>
            <a href="clientarea.php?action=productdetails&id={$serviceid}" style="color:#6b7280;font-size:13px;font-weight:600;text-decoration:none;margin-left:4px;"><i class="fas fa-arrow-left"></i></a>
        </div>
    </div>
    ...
    <div class="hchat-input-area">
        <div class="hchat-input-box">
            <textarea class="hchat-textarea" id="hchat-input" rows="1" placeholder="Message {$agent_name}&#8230;"></textarea>
            <button class="hchat-send" id="hchat-send"><i class="fas fa-paper-plane"></i></button>
        </div>
        <div class="hchat-input-hint">Enter to send &middot; Shift+Enter for new line</div>
    </div>
...
<script>
var _HCHAT_SVC  = {$serviceid};
var _HCHAT_NAME = "{$agent_name|escape:'javascript'}";
var _HCHAT_AJAX = 'modules/servers/hermesagent/ajax.php';
</script>
{literal}
<script>
(function () {
    var SVC_ID     = _HCHAT_SVC;
    var AGENT_NAME = _HCHAT_NAME;
    var AJAX       = _HCHAT_AJAX;
